...
Fundamentally, it wraps Connection and Pool objects from [mysql](https://www.npmjs.com/package/mysql) and adds convenience methods for common tasks and interaction patterns.
It was inspired by [postgres-easy](https://github.com/mvolkmann/postgresql-easy).

Migrating from 1.x to 2.x?  Please see: [Migrating](#migrating-from-1x-to-2x)
...
At this point, mysql-easier will have created a global connection pool for your database interactions using the configuration provided.  To get a connection from the pool (including from other modules):
...
If you would rather manage the connection pools yourself, you can create your own.
...
The config object in the methods above takes the same options as described in the [mysql documentation](https://github.com/mysqljs/mysql#connection-options).  These include:
...
#### `createConnection`
Creates a new MySqlConnection using a non-pooled connection with the provided configuration.
...
For example, if you require multiple simultaneous pools to exist at the same time.
...
This should only be done when you are confident that the pool is no longer necessary.

After you end the global pool, you can create a new one by calling `mySqlEasier.configure(config)` again.
...
Once you are done with the connection, please call `myConnection.done()` to release it back to the pool.
...
The basic benefits it provides are:
  1. `query` and other database methods return promises instead of taking callbacks.
  2. Provides a number of convenience methods for interacting with tables that have an auto-incremented primary key named `id`.

If you need to do some more advanced operations (like [streaming result rows](https://github.com/mysqljs/mysql#streaming-query-rows)), you can access the [mysqljs](https://github.com/mysqljs/mysql) 'Connection' by going directly to the `connection` property of this object.

Generally speaking, you will create a MySqlConnection directly (but you can if you like).
You will usually get connections by:
...
If the connection is pooled, the pool will create a new connection to take its place.
...
```js
try {
  const newId = await connection.insert('flavors', {name: 'vanilla', calories: 100});
...
```js
try {
  const rows = await connection.query('select name from flavors where calories < 150');
  // Process data in the array rows.
} catch (e) {
  // Handle the error.
}
...
The function must return a promise to indicate when it has completed (`resolve`) or when an error has occurred (`reject`).
...
```js
try {
  const rows = await connection.updateById('flavors', 7, {name: 'chocolate', calories: 200});
...
The basic benefits it provides are:
  ...
  2. Connections that are created are instances of our own MySqlConnection - which provides convenience methods for common database operations.

If you require additional control, you can access the mysqljs 'Pool' by going directly to the `pool` property of this object.

In most situations, we suspect that you will be using the global pool instead of interacting with this class.
...
#### `end`
End this pool.
After the end operation is complete, connections that were retrieved from this pool will no longer function.
...
When you are done with the connection, please call `myConnection.done()` to release it back to the pool.
...
If you are coming from mysql-easier 1.x, there are a few things you need to know.
...
The MySqlConnection that is returned is mostly the same as the API you have come to know and love except for how you end the connection.

### Getting a [mysqljs](https://github.com/mysqljs/mysql) Connection

In 1.x if you needed access to the connection instance as provided my 'mysql', you would request one this way:
...
In 2.x, connections are closed with the `done` method and the global pool is closed with `endPool`.
...
In most cases, there is no need to `await` the `Promise` returned from `connection.done()` unless you want to verify that the connection is released or closed successfully.
Likewise, Node will not cleanly exit your application if the pool is still running, so you don't really need to `await` the `Promise` from `mySqlEasier.endPool()` unless you want to make sure that it exits cleanly.